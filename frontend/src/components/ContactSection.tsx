...
import { Mail, Phone, MapPin, Github, Users, Loader2 } from 'lucide-react'; // Removed MessageSquare, Added Loader2
...
export function ContactSection() {
  ...
  const developers = [
    ...
    { name: 'Gauri Pansare', role: 'Frontend Developer', image: 'https://i.ibb.co/84XPdRcH/1733760879514.jpg', linkedin: 'https://www.linkedin.com/in/gauri-pansare-833a9a277/', github: 'https://github.com/' }
  ];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log(`Sending contact message request to: ${contactApiUrl}`); // Log for debugging
    // ---

    try {
      const response = await fetch(contactApiUrl, { // Use the constructed URL
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        const errorMessage = result.detail || `Error: ${response.statusText} (${response.status})`;
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
            <div className="bg-gray-900/60 backdrop-blur-sm rounded-xl border border-gray-800 p-8 mb-8">
              ...
              <form onSubmit={handleSubmit}>
                 {/* Form fields with value/onChange/disabled */}
                <div className="mb-4">
                ...
                <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-3 px-4 rounded-lg transition duration-200 flex items-center justify-center disabled:opacity-60 disabled:cursor-not-allowed" disabled={isSubmitting}>
                  {isSubmitting ? ( <> <Loader2 size={20} className="animate-spin mr-2" /> <span>Sending...</span> </> ) : ( <span>Send Message</span> )}
                </button>
              </form>
            </div>
          </div>
          ...
          <div>
            {/* Contact Info Box */}
            <div className="bg-gray-900/60 backdrop-blur-sm rounded-xl border border-gray-800 p-8 mb-8">
              {/* ... (Contact Info details remain the same) ... */}
            ...
            <div className="bg-gradient-to-r from-emerald-900/20 to-blue-900/20 rounded-xl border border-gray-800 p-8">
               {/* ... (WhatsApp link remains the same) ... */}
        ...
        <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl border border-gray-700 p-8 mt-16">
           {/* ... (GitHub project link remains the same) ... */}
